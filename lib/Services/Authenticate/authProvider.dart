...
class AuthProvider with ChangeNotifier {
  ...
  Future<void> updateTrustedContact(List<TrustedContacts> contacts) async {
    ...
    Future<void> createContact(String name, String phone) async {
      try {
        ...
        final response = await http.post(
            trustedContactAPI, headers: {"Authorization": header}, body: body);
      }
      catch (e) {
        print(e);
      }
    ...
    Future<bool> refreshOTP(String method, String email) async {
      ...
      if (response.statusCode == 200) {
        _notification =
            AuthStatusText('New OTP sent successfully. Please check!',
                type: 'info');
        notifyListeners();
        return true;
      }
    ...
    logOut([bool tokenExpired = false]) async {
      ...
      if (tokenExpired == true) {
        _notification =
            AuthStatusText(
                'Session expired. Please log in again.', type: 'info');
      }
      notifyListeners();
